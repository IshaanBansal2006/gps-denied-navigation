"""
LSTM v13: velocity-magnitude-weighted loss to fix z-axis regression.

Problem with v12: dense supervision assigns equal loss weight to every timestep.
Most timesteps have low velocity (hovering, slow flight, near-constant altitude),
so the model learns to predict "velocity ≈ mean" and gets penalized heavily on the
rare dynamic moments where velocity actually changes.

Fix: weight each timestep's loss by normalized velocity magnitude.
  weight(t) = ||v(t)|| / mean(||v||_sequence)
Timesteps where the drone is moving fast contribute more to the gradient.
Timesteps where the drone is near-static contribute proportionally less.

This is applied to the MSE component only. The cosine (directional) component
already masks near-zero vectors via min_norm, so it's already implicitly weighted.

Changes from v12:
  - VelocityWeightedDirectionalMSELoss replaces DirectionalMSELoss
  - Everything else identical (same LSTM arch, chunk_len=400, hidden_size=128)

Saves:
  checkpoints/lstm_v13.pt
  results/lstm_v13/{loss_history,test_metrics,normalization_stats}.json
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "lstm_v13"
...
class VelocityWeightedDirectionalMSELoss(nn.Module):
    """
    MSE weighted by velocity magnitude at each timestep, plus directional loss.

    weight(t) = ||v(t)|| / (mean(||v||) + eps)
    This downweights near-static timesteps and upweights dynamic motion.
    Mean weight ≈ 1 so overall loss scale is stable.
    """
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        # pred/target: (batch, seq_len, 3) in normalised space
        # Compute per-timestep velocity magnitude from TARGET (not pred, to avoid feedback)
        v_norm = target.norm(dim=-1)  # (batch, seq_len)
        weights = v_norm / (v_norm.mean() + 1e-8)  # normalise so mean≈1

        # Weighted MSE per timestep
        diff_sq = (pred - target).pow(2).mean(dim=-1)  # (batch, seq_len)
        weighted_mse = (weights * diff_sq).mean()

        # Directional loss on moving timesteps only (flatten time into batch)
...
def normalise_X(x_train, x_val, x_test):
    ...
    return n(x_train), n(x_val), n(x_test), mean, std


def normalise_y(y_train, y_val, y_test):
    flat = y_train.reshape(-1, 3)
    mean, std = flat.mean(0), flat.std(0) + 1e-8
    n = lambda y: (y - mean) / std
    return n(y_train), n(y_val), n(y_test), mean, std


def build_split(seqs):
    X_all, y_all = [], []
    for seq in seqs:
        result = load_seq(seq)
        if result is None:
            continue
        X_c, y_c = make_chunks(*result)
        print(f"  {seq}: {len(X_c)} chunks")
        X_all.append(X_c); y_all.append(y_c)
    return np.concatenate(X_all), np.concatenate(y_all)


def build_eval_split(seq_name):
...
def compute_metrics(model, loader, device):
    ...
    t_var = t.var(dim=0)
    axis_r2 = [float(1 - axis_mse[i] / t_var[i].item()) if t_var[i].item() > 1e-8 else float("nan")
               for i in range(3)]
...
def main():
    ...
    val_r = build_eval_split(VAL_SEQ); assert val_r is not None
    x_val, y_val = val_r
    print(f"Loading test: {TEST_SEQ}")
    test_r = build_eval_split(TEST_SEQ); assert test_r is not None
    x_test, y_test = test_r
    ...
    print("Loss: velocity-magnitude-weighted DirectionalMSE")
    ...
    criterion = VelocityWeightedDirectionalMSELoss(alpha=0.6)
    optimizer = torch.optim.Adam(model.parameters(), lr=3e-4, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100, eta_min=1e-6)

    epochs, patience = 100, 25
    best_val, no_improve = math.inf, 0
    best_ckpt = CHECKPOINT_DIR / "lstm_v13.pt"
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    history: Dict[str, List[float]] = {"train_loss": [], "val_loss": []}
    cfg = {"model": "lstm", "input_size": 6, "hidden_size": 128, "num_layers": 2,
           "output_dim": 3, "dropout": 0.3, "chunk_len": CHUNK_LEN,
           "loss": "velocity_weighted_directional_mse_alpha0.6",
    ...
    for epoch in range(1, epochs + 1):
        ...
        if improved:
            ...
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(),
                        ...
                        "norm_stats": norm_stats}, best_ckpt)
        else:
            no_improve += 1
    ...
    for ver, path in [("v12", "results/lstm_v12/test_metrics.json"),
                      ("v11", "results/tcn_v11/test_metrics.json")]:
        p = PROJECT_ROOT / path
        if p.exists():
            with open(p) as f:
                ref = json.load(f)
