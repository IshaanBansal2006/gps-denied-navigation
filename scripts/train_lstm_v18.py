"""LSTM v18 — bigger model + curriculum + val_final checkpoint selection.

Changes from v15:
  - Architecture:  hidden 128 → 256, layers 2 → 3, dropout 0.3 → 0.4
    (~800k params vs ~200k for v15). More capacity to capture curved
    trajectories like the swooping motion in MH_05 30s window.
  - Curriculum:    OUTAGE_LEN grows over training
                     epochs 1-15  → 2000  (10s)
                     epochs 16-30 → 4000  (20s)
                     epochs 31+   → 6000  (30s)
    Decision 027 noted v15's val_final dropped well past the val_mean min;
    curriculum should let the bigger model converge on the long-horizon target
    without diverging early.
  - Selection:     min(val_final) instead of min(val_mean). v15's loss-history
    figure showed val_final continuing to improve after val_mean plateaued.
  - LR:            5e-5 → 3e-5 (gentler for 4x larger network)
  - PATIENCE:      15 → 20 (curriculum transitions cause temporary val spikes)

Saves:
  checkpoints/lstm_v18.pt          (immutable; lstm_v15.pt untouched)
  results/lstm_v18/{test_metrics, normalization_stats, loss_history}.json
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "lstm_v18"

TRAIN_SEQS = ["MH_01_easy", "MH_02_easy", "MH_03_medium",
...
# Curriculum: (final_epoch, outage_len_samples)
CURRICULUM = [
    (15, 2000),   # epochs 1-15: 10s outages
    (30, 4000),   # epochs 16-30: 20s outages
    (60, 6000),   # epochs 31+: 30s outages (production target)
]
WARMUP_LEN = 2000
ACCUMULATE = 2
STRIDE = 25
...
def current_outage_len(epoch: int) -> int:
    for end_epoch, length in CURRICULUM:
        if epoch <= end_epoch:
            return length
...
class LSTMRegressor(nn.Module):
    def __init__(self, input_size=6, hidden_size=HIDDEN_SIZE,
                 num_layers=NUM_LAYERS, output_dim=3, dropout=DROPOUT) -> None:
        super().__init__()
...
def load_norm_stats() -> Dict:
    # Reuse v13's normalization stats — they're computed from the same
    # train split and the model input distribution hasn't changed.
...
def load_seq_raw(seq_name: str) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    ...
    if missing:
        ...
        return None
    return (df[IMU_COLS].to_numpy(dtype=np.float32),
            df[VEL_COLS].to_numpy(dtype=np.float32))
...
def train_step(
    ...
    for _ in range(ACCUMULATE):
        ...
        errors = (vel_filter[1:] - gt_denorm).norm(dim=-1)
...
def validate(
    ...
    """Always validates on full 30s rollout regardless of training curriculum."""
    model.eval()
...
def compute_chunk_metrics(
    ...
    model.eval()
    CHUNK_LEN, CHUNK_STRIDE = 400, 200
    T = len(imu_norm)
    preds, targets = [], []
...
def main() -> None:
    ...
    print(f"Selection criterion: min(val_final)\n")
    ...
    best_val_final, no_improve = math.inf, 0
    best_ckpt = CHECKPOINT_DIR / "lstm_v18.pt"
    ...
    cfg = {
        ...
        "dropout": DROPOUT, "loss": "nav_loss_mean_velocity_error_with_curriculum",
        "curriculum": CURRICULUM, "warmup_len_s": WARMUP_LEN / 200,
        "accumulate": ACCUMULATE, "lr": LR, "train_seqs": TRAIN_SEQS,
        "selection_criterion": "val_final",
    }

    history: Dict[str, List] = {"train_loss": [], "val_mean_err": [],
                                "val_final_err": [], "outage_len_per_epoch": []}

    for epoch in range(1, MAX_EPOCHS + 1):
        outage_len = current_outage_len(epoch)
        history["outage_len_per_epoch"].append(outage_len)

        epoch_loss = sum(
            train_step(model, seqs_norm, optimizer, device, y_mean_t, y_std_t,
                       rng, outage_len)
        ...
        improved = val_final < best_val_final
        if improved:
            best_val_final, no_improve = val_final, 0
            torch.save({
                "epoch": epoch, "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
                "best_val_final_err": best_val_final,
                "best_val_mean_err": val_mean,
                "config": cfg, "norm_stats": norm_stats_out,
            }, best_ckpt)
        else:
            no_improve += 1

        print(f"Epoch {epoch:03d} | outage {outage_len/200:.0f}s | "
              f"train {epoch_loss:.4f} | val_mean {val_mean:.4f} | "
              f"val_final {val_final:.4f}" + (" | best" if improved else ""))
    ...
    test_metrics = {
        **chunk_metrics,
        ...
        "best_val_final_err": best["best_val_final_err"],
    ...
    print(f"\nBest epoch: {best['epoch']}  "
          f"val_final: {best['best_val_final_err']:.4f} m/s  "
          f"val_mean: {best['best_val_mean_err']:.4f} m/s")
    print("Test metrics:")
