"""
TCN v8: scale model to [32,64,64] with absolute velocity target.

Hypothesis: v7 confirmed absolute velocity is a learnable target (r2_mean=+0.095).
With a well-defined target, more model capacity should improve accuracy further.

Changes from v7:
  - channel_sizes=[32, 64, 64] (~4x more params than v7's [16,32,32])
  - dropout reduced to 0.2 (less regularization needed with a meaningful target)
  - Everything else identical to v7: absolute velocity target, z-score normalised,
    directional loss alpha=0.6, balanced sampler, batch_size=64, lr=3e-4, cosine schedule

Saves:
  checkpoints/tcn_v8.pt
  results/tcn_v8/{loss_history,test_metrics}.json
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "tcn_v8"
...
def main() -> None:
    ...
    no_improve = 0
    patience = 25
    best_ckpt = CHECKPOINT_DIR / "tcn_v8.pt"
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    history: Dict[str, List[float]] = {"train_loss": [], "val_loss": []}
    cfg = {
        "input_channels": 6, "output_dim": 3,
        "channel_sizes": [32, 64, 64], "kernel_size": 3, "dropout": 0.2,
