#!/usr/bin/env python3
"""
Neural-aided EKF using TCN v7 absolute velocity predictions.
...
Compares vs dead reckoning, standalone TCN v7, EKF+GPS upper bound.

Usage:
    python3 scripts/neural_aided_ekf_v7.py [--seq SEQ] [--outages 5,10,30,60]
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "neural_aided_ekf_v7"
...
def load_v7(device: torch.device) -> tuple[TCNRegressor, dict]:
    ckpt_path = PROJECT_ROOT / "checkpoints" / "tcn_v7.pt"
    ...
    with open(PROJECT_ROOT / "results" / "tcn_v7" / "normalization_stats.json") as f:
        ns = json.load(f)
    norm = {
...
def main():
    ...
    model, norm = load_v7(device)
    print(f"Loaded tcn_v7.pt: {sum(p.numel() for p in model.parameters()):,} params")
