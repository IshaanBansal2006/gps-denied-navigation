"""
TCN v3: directional loss on SMALL model — isolate loss vs model-size.

Hypothesis: v2 regressed because [64,128,256,256] (~800k params) is massively
over-parameterized for 5548 windows. Directional loss itself may be fine.

Changes from v2:
  - Model: [16, 32, 32] (same as multi-seq baseline, ~50k params)
  - Everything else identical to v2: directional loss alpha=0.6, balanced
    sampler, batch_size=64, lr=3e-4, cosine schedule, patience=25

If v3 beats multi-seq (best_epoch > 1, test MSE < 0.08914, r2_mean > 0):
  → directional loss is good, model size was the v2 culprit
If v3 still overfits immediately (best_epoch = 1):
  → directional loss itself is the problem (cosine term unstable near zero)

Saves:
  checkpoints/tcn_v3.pt
  results/tcn_v3/{loss_history,test_metrics}.json
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "tcn_v3"
...
class DirectionalMSELoss(nn.Module):
    """0.6*MSE + 0.4*cosine dissimilarity (only where |target| > min_norm)."""
...
def load_per_seq_y() -> list[np.ndarray]:
    result = []
...
def main() -> None:
    ...
    model = TCNRegressor(
        input_channels=6,
        output_dim=3,
        channel_sizes=[16, 32, 32],   # small — same as multi-seq baseline
        kernel_size=3,
        dropout=0.3,
    ).to(device)
    ...
    no_improve = 0
    patience = 25
    best_ckpt = CHECKPOINT_DIR / "tcn_v3.pt"
