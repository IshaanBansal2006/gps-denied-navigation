"""
TCN v11: longer window (400 samples / 2s) + deeper network for larger receptive field.

Hypothesis: 1s window (200 samples) may not provide enough temporal context to distinguish
velocity patterns. At 200Hz the RF of the 3-layer v7 TCN is only RF=29 samples (~0.15s).
Doubling the window to 2s and adding depth:
  - WINDOW_SIZE = 400 (2s at 200Hz)
  - channel_sizes = [16, 32, 32, 32, 32, 32] (6 layers, dilations [1,2,4,8,16,32])
  - RF = 1 + 4*(1+2+4+8+16+32) = 253 samples (~1.26s) — covers 63% of 2s window

The 2x longer window increases per-window information even beyond the direct RF coverage:
the model sees a longer stretch of motion context at each stride step.

Changes from v7:
  - WINDOW_SIZE: 200 → 400
  - channel_sizes: [16,32,32] → [16,32,32,32,32,32]
  - No augmentation (v10 showed noise-only is neutral)

Saves:
  checkpoints/tcn_v11.pt
  results/tcn_v11/{loss_history,test_metrics,normalization_stats}.json
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "tcn_v11"
...
def main() -> None:
    ...
    print(f"Window size: {WINDOW_SIZE} samples ({WINDOW_SIZE/200:.1f}s)")

    sampler = build_balanced_sampler(per_seq_y)
    batch_size = 64
    train_loader = make_loader(x_train_n, y_train_n, batch_size, sampler=sampler)
    val_loader = make_loader(x_val_n, y_val_n, batch_size)
    test_loader = make_loader(x_test_n, y_test_n, batch_size)

    # 6 layers: dilations [1,2,4,8,16,32] → RF = 1 + 4*(63) = 253 samples (~1.26s)
    model = TCNRegressor(
        input_channels=6, output_dim=3,
        channel_sizes=[16, 32, 32, 32, 32, 32], kernel_size=3, dropout=0.3,
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"Parameters: {n_params:,}  (RF: 253 samples / 1.26s)")
    ...
    no_improve = 0
    patience = 25
    best_ckpt = CHECKPOINT_DIR / "tcn_v11.pt"
    ...
    # Compare key metrics vs v7 baseline
    v7_metrics_path = PROJECT_ROOT / "results" / "tcn_v7" / "test_metrics.json"
    if v7_metrics_path.exists():
        with open(v7_metrics_path) as f:
            v7 = json.load(f)
        print("\n--- Comparison vs v7 (baseline) ---")
        for k in ["r2_mean", "corr_x", "corr_y", "corr_z"]:
            delta = test_metrics[k] - v7[k]
            sign = "+" if delta >= 0 else ""
            print(f"  {k}: {test_metrics[k]:.4f} (v7: {v7[k]:.4f}, {sign}{delta:.4f})")
