"""
TCN v9: on-the-fly data augmentation — yaw rotation + noise + reflection.

Hypothesis: EuRoC sequences cover a limited range of headings (fixed corridors / room
layouts). The TCN overfits to these specific orientations. Yaw rotation augmentation
makes the model heading-invariant and multiplies effective training diversity.

Augmentations applied per-window during training (never val/test):
  1. Yaw rotation: R_yaw(θ) applied to gyro, accel, AND velocity label.
     Simulates same flight at a different compass heading. θ ~ Uniform[0, 2π].
  2. Gaussian noise: δ_gyro ~ N(0, 0.001²), δ_accel ~ N(0, 0.01²).
     Simulates sensor measurement variation.
  3. Axis reflection: flip x or y with p=0.3 each.
     Mirrors trajectory; valid for symmetric environments.

Changes from v7:
  - AugmentedDataset replaces TensorDataset for train loader
  - Val/test loaders unchanged (no augmentation)
...
Saves:
  ...
  results/tcn_v9/{loss_history,test_metrics}.json
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "tcn_v9"
...
SIGMA_GYRO_NOISE = 0.001   # rad/s
SIGMA_ACCEL_NOISE = 0.01   # m/s²
REFLECT_PROB = 0.3         # per axis
...
def yaw_rotation_matrix(theta: float) -> np.ndarray:
    c, s = np.cos(theta), np.sin(theta)
    return np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]], dtype=np.float32)


def augment_window(x: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Apply random yaw rotation, noise, and axis reflection to one window."""
    x = x.copy()
    y = y.copy()

    # 1. Yaw rotation — rotates both IMU (body frame) and velocity (world frame)
    R = yaw_rotation_matrix(np.random.uniform(0, 2 * np.pi))
    x[:, :3] = (R @ x[:, :3].T).T   # gyro
    x[:, 3:] = (R @ x[:, 3:].T).T   # accel
    y = R @ y                         # velocity label

    # 2. Gaussian noise on IMU
    x[:, :3] += np.random.randn(*x[:, :3].shape).astype(np.float32) * SIGMA_GYRO_NOISE
    x[:, 3:] += np.random.randn(*x[:, 3:].shape).astype(np.float32) * SIGMA_ACCEL_NOISE

    # 3. Axis reflection (flip x or y)
    for axis in [0, 1]:
        if np.random.rand() < REFLECT_PROB:
            x[:, axis] = -x[:, axis]       # gyro axis
            x[:, axis + 3] = -x[:, axis + 3]  # accel axis
            y[axis] = -y[axis]             # velocity axis
...
def main() -> None:
    ...
    print(f"Augmentations: yaw rotation + noise (σ_gyro={SIGMA_GYRO_NOISE}, σ_accel={SIGMA_ACCEL_NOISE}) + reflection")
    ...
    no_improve = 0
    patience = 25
    best_ckpt = CHECKPOINT_DIR / "tcn_v9.pt"
    ...
    cfg = {
        ...
        "augmentation": {
            "yaw_rotation": "uniform[0, 2pi]",
            "sigma_gyro_noise": SIGMA_GYRO_NOISE,
            "sigma_accel_noise": SIGMA_ACCEL_NOISE,
