...
RESULTS_DIR = PROJECT_ROOT / "results" / "neural_aided_ekf_lstm_v13"
...
def load_lstm_v13(device):
    ckpt = torch.load(PROJECT_ROOT / "checkpoints" / "lstm_v13.pt",
...
def main():
    ...
    model, norm = load_lstm_v13(device)
    print(f"Loaded lstm_v13.pt: {sum(p.numel() for p in model.parameters()):,} params")
