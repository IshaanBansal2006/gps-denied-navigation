"""
LSTM v15: end-to-end navigation loss, 30s simulated GPS outage.

Same as v14 but trains directly on 30s outages — matching the evaluation condition exactly.
Longer rollout = noisier gradient, so lr is halved and batch accumulation is 2 (not 4).

Changes from v14:
  - OUTAGE_LEN = 6000 (30s)
  - WARMUP_LEN = 2000 (10s — more context for longer outage)
  - ACCUMULATE = 2
  - LR = 5e-5
  - PATIENCE = 15 (fewer epochs needed since each step is 3× more compute)

Saves:
  checkpoints/lstm_v15.pt
  results/lstm_v15/{test_metrics,normalization_stats,loss_history}.json
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "lstm_v15"
...
OUTAGE_LEN = 6000       # 30s @ 200 Hz
WARMUP_LEN = 2000       # 10s pre-outage warmup
ACCUMULATE = 2
STRIDE = 25
SIGMA_PROCESS = 0.5
...
MAX_EPOCHS = 50
PATIENCE = 15
STEPS_PER_EPOCH = 100   # fewer steps per epoch (each step is 3× longer rollout)
VAL_OUTAGE_LEN = 6000
...
def main() -> None:
    ...
    print(f"Loss: pure nav loss, {OUTAGE_LEN/200:.0f}s outage")
    ...
    best_ckpt = CHECKPOINT_DIR / "lstm_v15.pt"
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        epoch_loss = sum(
            train_step(model, seqs_norm, optimizer, device, y_mean_t, y_std_t, rng)
        ...
        print(f"Epoch {epoch:03d} | train {epoch_loss:.4f} | "
