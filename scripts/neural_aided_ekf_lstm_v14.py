...
RESULTS_DIR = PROJECT_ROOT / "results" / "neural_aided_ekf_lstm_v14"
...
def load_lstm_v14(device):
    ckpt = torch.load(PROJECT_ROOT / "checkpoints" / "lstm_v14.pt",
...
def main():
    ...
    model, norm = load_lstm_v14(device)
    print(f"Loaded lstm_v14.pt: {sum(p.numel() for p in model.parameters()):,} params")
    ...
    labels = ["DR", "LSTM-v14"] + [f"EKF σ={s}" for s in SIGMA_TCN_SWEEP] + ["VelFilter", "EKF+GPS"]
